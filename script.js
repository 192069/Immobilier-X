...
(function () {
  ...
  const SOURCES = [
    {
      // Source la plus fiable : les fichiers sont dans le dépôt, donc servis
      // par le même site. Aucune restriction navigateur possible.
      nom: 'Données du dépôt (local)',
      async recuperer({ lat, lon, dist, citycode, annees }) {
        if (!citycode) throw new Error('code commune inconnu');
        const courante = new Date().getFullYear();

        // Fichier unique par commune, produit par le workflow GitHub Actions.
        // Les noms avec année restent acceptés si le fichier a été déposé à la main.
        const chemins = [`data/${citycode}.csv`];
        for (let a = courante; a > courante - annees - 2; a--) {
          chemins.push(`data/${citycode}-${a}.csv`);
        }
        ...
        const resultats = await Promise.all(
          chemins.map(async (chemin) => {
            try {
              ...
              // GitHub Pages renvoie la page 404 en HTML si le fichier manque
              if (/^\s*</.test(texte)) return [];
              const ventes = regrouperMutations(parseCSV(texte));
              journal.push(`${chemin.replace('data/', '')} : ${ventes.length}`);
              return ventes;
            } catch (e) {
              return [];
            }
        ...
        if (!toutes.length) {
          throw new Error(
            `commune ${citycode} absente de data/ — élargis rayon_km dans dvf.config.json, puis relance le workflow`
          );
        }
